...
import se.flightplanner.MovingMap.Clickable;
...
public class MapDrawer {
	...
	public Paint thinlinepaint;
	public Paint trippaint;
	public Paint seltrippaint;
	...
	public Vector getArrow(int xsize,int ysize,boolean dragging)
	{
	...
	private int top;
	private int bottom;
	DrawResult draw_actual_map(final MovingMap movingMap,Canvas canvas, int sizex, int sizey,
			boolean extrainfo,int extrainfolineoffset,boolean isDragging,
			TripData tripdata,
			TripState tripstate,
			AirspaceLookup lookup,
			Location lastpos,
			int zoomlevel,
			long last_real_position, //timestamp
			...
			)		
	{
		...
		Vector arrow=getArrow(right-left,bottom-top,isDragging);
		...
		if (bitmaps!=null)
		{
			...
			for(int j=0;j<tot;++j)
			{
				...
				for(int i=0;i<tot;++i)
				{
					...
					if (!movingMap.tileOnScreen((float)v.x,(float)v.y,tf))
						continue;
					BitmapRes b=null;
		...
		if (!isDragging)
		{
			...
			if (lastpos.hasSpeed())
			{
				...
				fivemin=(int)Project.approx_scale(mypos.y, zoomlevel, fivemindist);
			}
			canvas.drawRect((int)arrow.x,(int)arrow.y-fivenm, (int)arrow.x+6,(int)arrow.y-fivenm+4,arrowpaint);
			canvas.drawRect((int)arrow.x-10,(int)arrow.y-fivemin, (int)arrow.x,(int)arrow.y-fivemin+4,arrowpaint);
			...
			path.close();
			canvas.drawPath(path,arrowpaint);
			canvas.drawRect((int)arrow.x,(int)arrow.y-fivenm+1, (int)arrow.x+5,(int)arrow.y-fivenm+3,arrowpaint);
			canvas.drawRect((int)arrow.x-9,(int)arrow.y-fivemin+1, (int)arrow.x,(int)arrow.y-fivemin+3,arrowpaint);
		...
		else
		{
			if (lastpos!=null)
			{
				...
				Path path=new Path();
				path.moveTo((int)(screenpos.x+10*left.x-2*forward.x),(int)(screenpos.y+10*left.y-2*forward.y));
				path.lineTo((int)(screenpos.x+10*right.x-2*forward.x),(int)(screenpos.y+10*right.y-2*forward.y));
				path.lineTo((int)(screenpos.x+15*forward.x),(int)(screenpos.y+15*forward.y));
				path.close();
				canvas.drawPath(path,arrowpaint);
				...
				path=new Path();
				path.moveTo((int)(screenpos.x+7*left.x),(int)(screenpos.y+7*left.y));
				path.lineTo((int)(screenpos.x+7*right.x),(int)(screenpos.y+7*right.y));
				path.lineTo((int)(screenpos.x+12*forward.x),(int)(screenpos.y+12*forward.y));
				path.close();
				canvas.drawPath(path,arrowpaint);
		...
		if (we!=null)
		{
			...
			whenstr=movingMap.fmttime(when);
	...
	*/			
			...
			movingMap.addTextIfFits(canvas, "1222.2nm", r, String.format("%.1fnm",we.getDistance()), y1, bigtextpaint);
			//canvas.drawText(String.format("%.1fnm",we.getDistance()), 2,y1,bigtextpaint);
			movingMap.addTextIfFits(canvas, "T:22:22", r, whenstr, y1, bigtextpaint);
			//canvas.drawText(String.format("T:%s",whenstr), 70,y1,bigtextpaint);
			movingMap.addTextIfFits(canvas, null, r, we.getTitle(), y1, bigtextpaint);
			//canvas.drawText(we.getTitle(), 140,y1,bigtextpaint);
			for(int i=0;i<actuallines-1;++i)
			{
				Log.i("fplan","Actuallines:"+actuallines+" offset: "+extrainfolineoffset+" details length:"+details.length);
				if (extrainfolineoffset+i<details.length)
					canvas.drawText(details[extrainfolineoffset+i], 2,y1+tsy+i*tsy,bigtextpaint);					
			}
			if (tripstate.hasLeft())
			{
				canvas.drawLine((int)(0+2.8f*x_dpmm),(int)(bottom-0.7f*y_dpmm),			
								(int)(0+2.0f*x_dpmm),(int)(bottom-1.5f*y_dpmm),thinlinepaint);
				canvas.drawLine((int)(0+2.0f*x_dpmm),(int)(bottom-1.5f*y_dpmm),			
								(int)(0+2.8f*x_dpmm),(int)(bottom-2.3f*y_dpmm),thinlinepaint);
			}
			else
			{
				canvas.drawLine(
						(int)(0+3.25f*x_dpmm),(int)(bottom-0.7f*y_dpmm),			
						(int)(0+1.75f*x_dpmm),(int)(bottom-2.3f*y_dpmm),thinlinepaint);
				canvas.drawLine(
						(int)(0+3.25f*x_dpmm),(int)(bottom-2.3f*y_dpmm),
						(int)(0+1.75f*x_dpmm),(int)(bottom-0.7f*y_dpmm),
						thinlinepaint);
				
			}
			canvas.drawRect(
					(int)(0.2f*x_dpmm),(int)(bottom-2.8f*y_dpmm), 
					(int)(4.8f*x_dpmm),(int)(bottom-0.2f*y_dpmm),
					thinlinepaint);
			float off=right-(5.0f*x_dpmm);
			if (tripstate.hasRight())
			{
				canvas.drawLine((int)(0+2.0f*x_dpmm+off),(int)(bottom-0.7f*y_dpmm),			
						(int)(0+2.8f*x_dpmm+off),(int)(bottom-1.5f*y_dpmm),thinlinepaint);
				canvas.drawLine((int)(0+2.8f*x_dpmm+off),(int)(bottom-1.5f*y_dpmm),			
								(int)(0+2.0f*x_dpmm+off),(int)(bottom-2.3f*y_dpmm),thinlinepaint);
				canvas.drawRect(
						(int)(0.2f*x_dpmm+off),(int)(bottom-2.8f*y_dpmm), 
						(int)(4.8f*x_dpmm+off),(int)(bottom-0.2f*y_dpmm),
						thinlinepaint);
			}
			{
				canvas.drawRect(
						(int)(5.2f*x_dpmm),(int)(bottom-2.8f*y_dpmm), 
						(int)(-0.2f*x_dpmm+off),(int)(bottom-0.2f*y_dpmm),
						thinlinepaint);
				String t;
				if (extrainfo)
				{
				...
				canvas.drawText(t,(right-left)*0.5f-tbounds.width()/2,(int)(bottom-1.0*y_dpmm),textpaint);
			}
		...
		canvas.drawRect(r, backgroundpaint);
		movingMap.addTextIfFits(canvas,"222°.",r,String.format("%03.0f°",lastpos.getBearing()),y,bigtextpaint);
		movingMap.addTextIfFits(canvas,"222kt",r,String.format("%.0fkt",lastpos.getSpeed()*3.6/1.852),y,bigtextpaint);
		...
		movingMap.addTextIfFits(canvas,"122:22",r,movingMap.fmttime(td),y,bigtextpaint);
		if (havefix) //if significantly after 1970-0-01
		{
			...
			movingMap.addTextIfFits(canvas,"FIX:222222Z",r,"FIX:"+formatter.format(d)+"Z",y,bigtextpaint);
		}
		else
		{
			//canvas.drawText("NOFIX",220,y,bigtextpaint);
			movingMap.addTextIfFits(canvas,"FIX:222222Z",r,"NOFIX",y,bigtextpaint);
		}
		///canvas.drawText(String.format("Z%d",zoomlevel), 0,y,bigtextpaint);
		movingMap.addTextIfFits(canvas,"Z13",r,String.format("Z%d",zoomlevel),y,bigtextpaint);
		...
		else
		{
			if (download_status!=null && !download_status.equals(""))
			{
				...
				canvas.drawText("Download:"+download_status,left+3,y2,bigtextpaint);			
			}
		}
	...
	public int getNumInfoLines(int ysize) {
		...
		if (maxlines<=1)
			throw new RuntimeException("The screen is too small for this program");
